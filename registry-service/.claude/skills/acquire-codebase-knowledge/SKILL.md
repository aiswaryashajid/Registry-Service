...
```
- [ ] Phase 0: Read graphify + cartographer outputs FIRST (graph-as-index)
- [ ] Phase 1: Run scan ONLY to fill gaps the graph/map don't cover, read intent docs
...
### Phase 0: Reuse the Graph and Map (Graph-First Mandate)

When this skill runs inside the KT pipeline, the structural analysis is
ALREADY done. Seed all seven documents from these existing outputs BEFORE
reading any source file:

- `graphify-out/GRAPH_REPORT.md` — the primary index: communities (→ STRUCTURE,
  ARCHITECTURE), god nodes (→ ARCHITECTURE, CONCERNS), hyperedges (→ ARCHITECTURE
  data flow), stack/deps (→ STACK).
- `docs/CODEBASE_MAP.md` (cartographer) — feature flows, module guide, conventions,
  gotchas (→ ARCHITECTURE, CONVENTIONS, CONCERNS).

Rules:
- Treat `GRAPH_REPORT.md` as a bounded INDEX. Do NOT load the full
  `graphify-out/graph.json` into context. When you need a specific node, edge,
  or path the report doesn't spell out, use the query tools instead of dumping
  the file:
    - `graphify query "<question>"`   (broad context)
    - `graphify path "<A>" "<B>"`     (shortest connection)
    - `graphify explain "<node>"`     (one node, plain language)
- Open source files ONLY to confirm a specific detail the graph and map do not
  already answer (e.g. exact `@Column` constraints, a config value). Never
  re-scan the whole tree when the graph already located the symbol.
- Graph-unavailable fallback: if `graphify-out/GRAPH_REPORT.md` is missing, say
  so explicitly ("graph unavailable — falling back to source") and only then run
  the full scan in Phase 1.
...
### Phase 1: Scan (Gap-Fill Only) and Read Intent

1. Run the scan script ONLY when Phase 0 left gaps the graph and map cannot
   answer (or when the graph is unavailable). The scan is a fallback for
   manifest/config discovery, not the default first move:
   ```bash
